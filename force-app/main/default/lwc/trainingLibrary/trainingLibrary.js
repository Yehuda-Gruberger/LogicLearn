...
export default class TrainingLibrary extends LightningElement {
  ...
  get adminTutorialsClass() {
    return this.adminNavClass("tutorials");
  }
  get adminGroupsClass() {
    return this.adminNavClass("groups");
  }
  get adminSettingsClass() {
    return this.adminNavClass("settings");
  }
  ...
  get folderItems() {
    ...
    const childrenOf = (pid) =>
      this.folders.filter((f) => (f.parentId || null) === (pid || null));
  ...
  get overallProgressLabel() {
    ...
    const done = this.sections.reduce(
      (sum, s) =>
        sum + s.videos.filter((v) => v.viewStatus === "Completed").length,
      0,
    );
    return {
      done,
    ...
    return this.sections
      ...
      .map((s) => {
        const videos = s.videos
          .filter(
            (v) => !term || (v.title && v.title.toLowerCase().includes(term)),
          )
          .map((v) => this.decorate(v));
        const doneCount = videos.filter(
          (v) => v.viewStatus === "Completed",
        ).length;
        return {
          key: s.folderId || "none",
  ...
  get selectedFolderName() {
    return (
      this.folderItems.find((item) => item.id === this.selectedFolderId)
        ?.name || "All Training"
    );
  }
  ...
  get hasRequiredVideos() {
    return this.requiredVideoCards.length > 0;
  }
  get hasVisibleVideos() {
    return this.visibleVideoCards.length > 0;
  }
  get requiredFinishLabel() {
    return `${this.requiredVideoCards.length} to finish`;
  }
  get allFilterClass() {
    return this.libraryFilter === "all" ? "filter-chip active" : "filter-chip";
  }
  get requiredFilterClass() {
    return this.libraryFilter === "required"
      ? "filter-chip active"
      : "filter-chip";
  }
  get progressFilterClass() {
    return this.libraryFilter === "progress"
      ? "filter-chip active"
      : "filter-chip";
  }
  get completedFilterClass() {
    return this.libraryFilter === "completed"
      ? "filter-chip active"
      : "filter-chip";
  }
  get allTypesClass() {
  ...
  renderedCallback() {
    ...
    const adminConsole = this.template.querySelector(
      "c-training-admin-console",
    );
    if (adminConsole) {
  ...
  syncAvailableHeight() {
    const app = this.template.querySelector(".app");
    if (!app) return;
    const top = Math.max(0, app.getBoundingClientRect().top);
    const available = Math.max(500, window.innerHeight - top - 8);
    app.style.setProperty("--logic-learn-available-height", `${available}px`);
  }
